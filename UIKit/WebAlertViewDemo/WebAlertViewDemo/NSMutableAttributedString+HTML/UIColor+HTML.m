...
+ (UIColor *)colorWithHexString:(NSString *)hex andAlpha:(CGFloat)alpha
{
    ...
    if ( ([hex length] - offset != 6) && ([hex length] - offset != 3))
	{
		return nil;
	}
	
	NSUInteger digits = ([hex length] - offset) / 3;
	CGFloat maxValue = (digits == 1)? 15.0:255.0;
	
	CGFloat red = [[hex substringWithRange:NSMakeRange(0 + offset, digits)] integerValueFromHex] / maxValue;
	CGFloat green = [[hex substringWithRange:NSMakeRange(digits + offset, digits)] integerValueFromHex] / maxValue;
	CGFloat blue = [[hex substringWithRange:NSMakeRange(2 * digits + offset, digits)] integerValueFromHex] / maxValue;
